...
resource "azurerm_hdinsight_spark_cluster" "adl_hdi_spark" {
  ...
  metastores {
    ambari {
      server        = join("", [var.ambari_database_name, ".database.windows.net"])
    ...
    hive {
      server        = join("", [var.hive_database_name, ".database.windows.net"])
    ...
    oozie {
      server        = join("", [var.oozie_database_name, ".database.windows.net"])
