...
variable "api_server_authorized_ip_ranges" {
  ...
  validation {
    condition     = length(var.api_server_authorized_ip_ranges) == 0 || alltrue([for v in var.api_server_authorized_ip_ranges : can(regex("^([0-9]{1,3}\\.){3}[0-9]{1,3}(\\/([0-9]|[1-2][0-9]|3[0-2])){1}$", v))])
