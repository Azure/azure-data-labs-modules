output "id" {
  value = (
    length(azurerm_batch_account.adl_batch) > 0 ?
    azurerm_batch_account.adl_batch[0].id : ""
  )
...
output "name" {
  value = (
    length(azurerm_batch_account.adl_batch) > 0 ?
    azurerm_batch_account.adl_batch[0].name : ""
  )
...
output "resource_group_name" {
  value = (
    length(azurerm_batch_account.adl_batch) > 0 ?
    azurerm_batch_account.adl_batch[0].resource_group_name : ""
  )
